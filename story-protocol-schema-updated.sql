...
-- URL: https://supabase.com/dashboard/project/wqbagygoqcprrlgredod/sql

-- 1. Create asset_metadata table
...
-- 5. Enable RLS (Row Level Security) for the tables
...
-- 6. Create RLS policies for asset_metadata
...
-- 7. Create RLS policies for ip_groups
...
-- 8. Create RLS policies for pil_terms
...
-- 11. Create trigger for updating timestamps
CREATE TRIGGER update_ip_groups_updated_at
    BEFORE UPDATE ON ip_groups
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

-- 12. Grant permissions for anonymous users (needed for the app to work)
...
-- 14. Grant usage on sequences
GRANT USAGE ON ALL SEQUENCES IN SCHEMA public TO anon;
GRANT USAGE ON ALL SEQUENCES IN SCHEMA public TO authenticated;

-- Success message
SELECT 'Story Protocol database schema created successfully!' as message;